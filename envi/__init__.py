...
class ArchitectureModule:
    ...
    def getOpRenderer(self):
        raise ArchNotImplemented("getOpRenderer")
...
class Operand:
    ...
    def __init__(self, mode, tsize=0, reg=None, indexreg=None, imm=None, disp=None, scale=None):
        self.tsize = tsize
        self.mode = mode
        self.reg = reg
...
class Opcode:
    ...
    """
    def __init__(self, opcode, mnem, prefixes, size, operands, iflags=0, cond=0):
        """
...
class Emulator(e_mem.IMemory):
    ...
    def getOperAddress(self, op, idx):
        ...
        psize = self.arch.getPointerSize()
        oper = op.opers[idx]

        if oper.mode == OM_IMMMEM:
            base,size = self.getSegmentInfo(op)
            return e_bits.unsigned(base + oper.imm, psize)
        ...
        if oper.mode == OM_REGMEM:
            ...
            return e_bits.unsigned(val, psize)
        ...
        if oper.mode == OM_SIBMEM:
            ...
            base,size = self.getSegmentInfo(op)
            return e_bits.unsigned(addr + base, psize)
