...
def _loadMacho(vw, filebytes, filename=None):
    ...
    baseaddr = vw.getOptionInt('macho', 'baseaddr')
    ...
    if filebytes[:4].encode('hex') in ('cafebabe', 'bebafeca'):
        ...
        archhdr = None
        fatarch = vw.getOption('macho', 'fat_arch')
        ...
        if not archhdr:
            ...
            raise Exception('Mach-O Fat Binary: Please specify arch with -O macho:fat_arch:<archname>')
