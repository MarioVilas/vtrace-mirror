...
class SymbolikEmulator:
    ...
    def applyFunctionCall(self, funcsym):
        '''
        This API allows arch/platform specific emulators to handle (and expand out)
        function calls to modify the emulator state.  A symbolik emulator by itself is
        *not* function aware, so this does nothing, but on SymbolikFunctionEmulator instances
        this will modify the machine state to account for the function call...
        '''
        pass
    ...
    def setSymVariable(self, name, symval):
        symval = frobSymbol(symval)
