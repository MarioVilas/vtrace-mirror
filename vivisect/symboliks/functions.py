...
def getSymbolikFunction(emu, funcsym):
    '''
    Return an instance of a calling convention proxy based on the given
    symbol.  Where possible, enumerate argument count/type or the presense
    of an import to emulate...
    '''
    ...
    if funcsym.isDiscrete(emu):
        ...
        if emu._sym_vw.isFunction(fva):
            ...
            ccname = emu._sym_vw.getFunctionMeta(fva, 'CallingConvention', 'default')
